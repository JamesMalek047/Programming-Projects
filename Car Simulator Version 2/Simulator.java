...
public class Simulator {
	...
	public int parkDuration(Spot car, int clock) {
		return clock - car.getTimestamp();
	}
	...
	public void simulate() {
		...
		Spot carWaitingToGetParked = null;
		...
		while (clock < steps) {
			...
			if (RandomGenerator.eventOccurred(probabilityOfArrivalPerSec)) {
				Spot newSpot = new Spot(RandomGenerator.generateRandomCar(), clock);
				incomingQueue.enqueue(newSpot);
			}

			for (int i = 0; i < lot.getNumRows(); i++) {
				for (int j = 0; j < lot.getNumSpotsPerRow(); j++) {
					Spot tempSpot = lot.getSpotAt(i, j);

					if (tempSpot != null) {
						boolean carLeaving = false;
						if (parkDuration(tempSpot, clock) > 28800) {
							carLeaving = true;

						} else {
							carLeaving = RandomGenerator.eventOccurred(departurePDF.pdf(parkDuration(tempSpot, clock)));
						}

						if (carLeaving) {
							Spot exitingCar = lot.remove(i, j);
							exitingCar.setTimestamp(clock);
							outgoingQueue.enqueue(tempSpot);
						}
					}
				}
			...
			if (carWaitingToGetParked != null) {
				// carWaitingToGetParked=incomingQueue.dequeue();
				if (lot.attemptParking(carWaitingToGetParked.getCar(), clock)) {
					System.out.println(carWaitingToGetParked.getCar() + " ENTERED at timestep " + clock + " ; occupancy is at " + lot.getTotalOccupancy());
					carWaitingToGetParked = null;
				}
			...
			if (!outgoingQueue.isEmpty()) {
				Spot carLeavingParkingSpot = outgoingQueue.dequeue();
				System.out.println(carLeavingParkingSpot.getCar() + " EXITED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
			}
			clock++;
		}
